...
*/
    ...
    // mongodb
    var mods = mongoose.modelNames();
    var modexists = false;
    for ( var key in mods ) {
      if ( modelname === mods[key].toLowerCase() ) {
        modexists = true;
      }
    }
    if (!modexists) {
      amodel = mongoose.model(get_class(this),{title: String,created_at: String});
    } else {
      amodel = mongoose.model(get_class(this));
    }
    
    amodel.find(function (err, data) {
      model.data = data;
      model.send( 'changed' );
//      for ( var bb in data ) {
//        rows.push({title:data[bb].title,created_at:data[bb].created_at});
//      }
//      console.log(rows);
...
function config( settings ) {
  ...
  if (!(typeof prefs['dbport'] == 'undefined'))
    pg = require('pg');
  ...
  if (pg != false) {
    ...
    mongoose.connect('mongodb://'+prefs['dbhost']+'/'+prefs['dbname']);
  ...
  if (pg != false) {
    ...
    req.on('end', function() {
      ...
      query.on('end', function(result) {
        ...
        var modnam = modelname.charAt(0).toUpperCase() + modelname.slice(1);
        if (!modexists) {
          amodel = mongoose.model(modnam,{title: String,created_at: String});
        } else {
          amodel = mongoose.model(modnam);
        }
